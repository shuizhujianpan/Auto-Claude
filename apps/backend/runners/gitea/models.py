"""
Gitea Automation Data Models
============================

Data structures for Gitea automation features.
Stored in .auto-claude/gitea/pr/
"""
...
class PRReviewResult:
    ...
    def save(self, gitea_dir: Path) -> None:
        """Save review result to .auto-claude/gitea/pr/"""
        pr_dir = gitea_dir / "pr"
        pr_dir.mkdir(parents=True, exist_ok=True)

        review_file = pr_dir / f"review_{self.pr_index}.json"
        with open(review_file, "w", encoding="utf-8") as f:
            json.dump(self.to_dict(), f, indent=2)

    @classmethod
    def load(cls, gitea_dir: Path, pr_index: int) -> PRReviewResult | None:
        """Load a review result from disk."""
        review_file = gitea_dir / "pr" / f"review_{pr_index}.json"
...
class GiteaRunnerConfig:
    """Configuration for Gitea automation runners."""

    # Authentication
    token: str
    repository: str  # owner/repo format
    instance_url: str = "https://gitea.com"
...
@dataclass
class FollowupPRContext:
    """Context for a follow-up PR review."""
